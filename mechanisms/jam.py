  ...
  def run(self, data, pub_data, workload):
    ...
    while t < self.rounds:
      ...
      if not measurements:  # First round
        priv_scores = [
            -1 * expected_priv_error(measurement_sigma, domain.size(marg))
            for marg in candidates_filtered
        ]
        pub_scores = [-1 * pub_errors[marg] for marg in candidates_filtered]
      else:
        model_error = {
            marg: np.linalg.norm(
        ...
        pub_scores = [
            model_error[marg] - pub_errors[marg] for marg in candidates_filtered
        ]
      ...
      if mtype == "PRIV":
        ...
        measurements.append(LinearMeasurement(y, selected_marg, stddev=1.0))
      ...
      model_size = junction_tree.hypothetical_model_size(domain, model.cliques)
...
if __name__ == "__main__":
  ...
  parser.add_argument("--dataset", type=str, help="dataset name (e.g., adult)")
  parser.add_argument(
      "--data_dir", type=str, help="base directory for datasets"
  )
  parser.add_argument("--epsilon", type=float, help="privacy parameter epsilon")
  parser.add_argument("--delta", type=float, help="privacy parameter delta")
  parser.add_argument("--alpha", type=float, help="selection budget proportion")
  parser.add_argument(
  ...
  parser.add_argument("--save_errors", type=str, help="path to save errors CSV")
